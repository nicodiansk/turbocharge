#!/bin/bash
# ABOUTME: Tests whether Claude triggers a skill based on a natural prompt
# ABOUTME: Runs Claude with --plugin-dir and checks stream-json output for Skill tool invocation
#
...
# Get the turbocharge plugin root (two levels up from tests/skill-triggering)
PLUGIN_DIR="$(cd "$SCRIPT_DIR/../.." && pwd)"

TIMESTAMP=$(date +%s)
OUTPUT_DIR="/tmp/turbocharge-tests/${TIMESTAMP}/skill-triggering/${SKILL_NAME}"
mkdir -p "$OUTPUT_DIR"
